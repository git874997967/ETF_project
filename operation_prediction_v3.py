...
class StrategyEngine:
    ...
    def backtest(self, leverage_ticker, start_date="2016-01-01", end_date="2026-02-27",
                 mode_a_rsi=55, mode_b_rsi=30, bbl_buffer=1.0, strict_gmma=False, vol_filter_pct=None,
                 mode_b_confirm=True):
        ...
        df_parent = yf.download(underlying, start=data_start, end=end_date, progress=False)
        df_child = yf.download(leverage_ticker, start=data_start, end=end_date, progress=False)
        # [新增] 下载 VIX 数据用于宏观对冲
        ...
        df.rename(columns={'close_child': 'close_levered', 'atr_14': 'atr_14_child', 'open_child': 'open_child', 'close_vix': 'vix_close'}, inplace=True)
        df = df.dropna().copy()

        # columns
        rsi_col = 'rsi_14'; bbl_col = 'bbl_20'; adx_col = 'adx_14'; atr_col_child = 'atr_14_child'
        ...
        # 实际仓位 = 基础 30% * 风险调节因子
        df.loc[cond_mode_a_t1, 'Suggested_Size'] = 0.3 * risk_factor
        ...
        # 实际仓位 = 目标总额 70% * 风险调节因子
        df.loc[cond_mode_a_t2, 'Suggested_Size'] = 0.7 * risk_factor
        ...
        if mode_b_confirm:
            ...
            cond_mode_b = (df[rsi_col] < mode_b_rsi) & (df['close'] < df[bbl_col] * bbl_buffer) & cond_bear_regime
    ...
    def analyze_result_df(self, df, trade_cost=0.001):
        ...
        if split_idx < 2:
            res['in_sample_apy'] = None; res['oos_apy'] = None
        else:
...
def main():
    p = argparse.ArgumentParser(description='operation_prediction v3 — StrategyEngine CLI')
    ...
    if args.action == 'run':
        res = engine.backtest(args.ticker, start_date=args.start, end_date=args.end,
                              mode_b_rsi=args.rsi, bbl_buffer=args.bbl)
        if not res:
            print('No results (empty data)')
            return
        summary = {k: v for k, v in res.items() if k in ('Ticker', 'Underlying', 'APY', 'Total_Return', 'UI_Summary')}
        ...
        for r in rsi_vals:
            for b in bbl_vals:
                res = engine.backtest(args.ticker, start_date=args.start, end_date=args.end, mode_b_rsi=r, bbl_buffer=b)
                if not res:
                    continue
                metrics = engine.analyze_result_df(res['Result_DF'])
                #rows.append({"rsi": r, "bbl": b, "apy": res['APY'], "num_trades": metrics.get('num_trades'), "max_dd": metrics.get('max_drawdown'), "sqn": metrics.get('sqn',0)})
                # 约束条件：交易次数 > 15 且 最大回撤 < 35%
                if metrics.get('num_trades', 0) >= 15 and metrics.get('max_drawdown', 0) > -0.35:
                    rows.append({
                        "rsi": r, "bbl": b, "sqn": metrics.get('sqn', 0), "num_trades": metrics.get('num_trades'),
                        "apy": res['APY'], "max_dd": metrics.get('max_drawdown')
                    })
        rows_sorted = sorted(rows, key=lambda x: x['sqn'], reverse=True)
